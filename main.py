...
async def quick_ad_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for target in data["targets"]:
        execute(
            ...
            VALUES (%s, %s, 'active', %s, %s, TRUE, %s, %s, NOW(), NULL, NULL)
    ...
    await update.message.reply_text(
        ...
        f"如需设置立即投放、开始时间、静默时间、投放期限，请使用：\n"
        f"/shezhi 目标\n\n"
        f"例如：/shezhi {data['targets'][0]}"
    )
...
"""
    ...
    logs = fetch_one("SELECT COUNT(*) AS c FROM delivery_logs;")["c"]
...
async def send_ad_to_target(bot, target_row, force=False):
    ...
    if not ad:
        log_delivery(target, None, None, "failed", "没有可投放广告", "请检查广告素材和广告主是否启用")
        return False, "没有可投放广告"
...
def main():
    ...
    print("广告投放机器人启动成功：支持快速广告、投放设置、日志、管理员通知")
    app.run_polling()
